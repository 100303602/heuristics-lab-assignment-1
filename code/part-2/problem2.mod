...
s.t. Location {i in CREW, j in FLIGHTS}: sum{k in MAD_VAL_FLIGHTS: k<j} x[k,i] - sum{l in VAL_MAD_FLIGHTS: l<j} x[l,i] <= 1;
s.t. Location2{i in CREW, j in FLIGHTS}: sum{k in VAL_MAD_FLIGHTS: k<j} x[k,i] - sum{l in MAD_VAL_FLIGHTS: l<j} x[l,i] <= 0;
